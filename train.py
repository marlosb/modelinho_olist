...
def train_xgb(steps, X_train, y_train):
    model_xgb = xgb.XGBClassifier(random_state=1992, verbosity = 0)
...
def get_models(DATA_PATH, model_path):
    ...
    # Regressão Logística
    best_model_rl = train_rl(imputer_zero, imputer_um, onehot, X_train, 
                             y_train)
    # Árvore de Decisão
    best_model_tree = train_tree(imputer_zero, imputer_um, onehot, 
                                 X_train, y_train)
    # Random Forest
    best_model_rf = train_forest(imputer_zero, imputer_um, onehot, 
                                 X_train, y_train)
    # XGBoost 1
    steps = [('zero', imputer_zero), ('um', imputer_um),
             ('onehot', onehot)]
